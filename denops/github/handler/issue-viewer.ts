...
async function setKeymap(denops: Denops, buf: Buffer) {
  await buffer.ensure(denops, buf.bufnr, async () => {
    ...
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-next)",
      args: [buf.bufnr, "next", {}],
    });
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-prev)",
      args: [buf.bufnr, "prev", {}],
    });
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-edit-cursor)",
      args: [buf.bufnr, "edit-cursor", {}],
    });
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-edit-body)",
      args: [buf.bufnr, "edit-body", {}],
    });
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-new-comment)",
      args: [buf.bufnr, "new-comment", {}],
    });
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-browse)",
      args: [buf.bufnr, "browse", {}],
    });
    await mapDispatch({
      ...opt,
      lhs: "<Plug>(denops-github-issue-viewer-browse-cursor)",
